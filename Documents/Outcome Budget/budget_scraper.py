...
class OutcomeBudgetScraper:
    ...
    def extract_table(self, soup, section_name, page_url):
        ...
        # Capture column headers for structural clarity
        ...
        for tr in table.select("tbody tr"):
            ...
            title = data.get("Title") or (cells[1] if len(cells) > 1 else "Untitled")

            pdf_tag = tr.select_one("a[href$='.pdf']")
            pdf_url = urljoin(BASE_URL, pdf_tag["href"]) if pdf_tag else ""

            # Combine structure and raw row content
            content_repr = " | ".join(f"{h}: {data.get(h, '')}" for h in headers)
    ...
    def handle_pagination(self, section_name, start_url):
        ...
        while True:
            ...
            WebHTML = self.driver.page_source
            soup = BeautifulSoup(WebHTML, "lxml")
            self.extract_table(soup, section_name, current_url)

            # Detect pagination links
            try:
                next_button = self.driver.find_element(By.CSS_SELECTOR, "li.pager__item--next a")
    ...
    def save_csv(self):
        ...
        out_path = os.path.join(OUT_DIR, f"outcome_budget_{ts}.csv")
